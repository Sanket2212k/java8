...
public class Trip_travel {

	public List<VarshaPackage> generatePackageCost() throws InvalidIdException, SQLException {
		List<VarshaPackage> list = new ArrayList();
		int i = 1;
		Scanner sc = null;
		try {
			Connection cnn=DBHandler.connectkk();
			...
			while (sc.hasNextLine()) {
				...
				if (validate(id)) {
					...
					VarshaPackage P1 = new VarshaPackage(id, source_place, destination_place,days,
							basic_fare);
					P1.TripCalculate();
					list.add(P1);
					ps.setString(1, id);
	...
	public List<VarshaPackage> findPackagesWithMinimumNumberOfDays() {

		List<VarshaPackage> list = new ArrayList();
		try {
			Connection connection = DBHandler.connectkk();
			...
			while (rs.next()) {
				...
				VarshaPackage v1 = new VarshaPackage(id, src, dstn, days, 0);
